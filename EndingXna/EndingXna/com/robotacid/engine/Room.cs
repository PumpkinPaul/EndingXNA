...
namespace com.robotacid.engine
{
    ...
    public class Room  {
        ...
        public readonly static int[] compass = {UP, RIGHT, DOWN, LEFT};
        ...
        //    8:3
        //};
        public static readonly Array<Point> compassPoints = new Array<Point> {new Point(0, -1), new Point(1, 0), new Point(0, 1), new Point( -1, 0)};
        ...
        public void connectDoors(Array<Point> doors) {
            Point p;
            int r, c, x, y;
            // N S
            x = doors[NORTH].x;
        ...
        public void clearCheck(int x, int y) {
            if(y > 0 && ((map[y - 1][x] & ENEMY) == ENEMY) && (
                    (
                        ...
                        ((map[y - 1][x] & (TRAP | MOVER)) > 0 ) && ((map[y - 1][x] & M_DOWN) == M_DOWN)
                    ) || (
            ...
            if(x < width - 1 && ((map[y][x + 1] & ENEMY) == ENEMY) && (
                    (
                        ...
                        ((map[y][x + 1] & (TRAP | MOVER))  > 0) && ((map[y][x + 1] & M_LEFT) == M_LEFT)
                    ) || (
            ...
            if(y < height - 1 && ((map[y + 1][x] & ENEMY) == ENEMY) && (
                    (
                        ...
                        ((map[y + 1][x] & (TRAP | MOVER)) > 0) && ((map[y + 1][x] & M_UP) == M_UP)
                    ) || (
                        ((map[y + 1][x] & GENERATOR) == GENERATOR)
                    )
                )
            ...
            if(x > 0 && ((map[y][x - 1] & ENEMY) == ENEMY) && (
                    (
                        ...
                        ((map[y][x - 1] & (TRAP | MOVER)) > 0) && ((map[y][x - 1] & M_RIGHT) == M_RIGHT)
                    ) || (
                        ((map[y][x - 1] & GENERATOR) == GENERATOR)
                    )
                )
        ...
        public void copyTo(Array<Array<int>> target, int tx, int ty) {
            int r, c;
            for(r = 0; r < height; r++){
                for(c = 0; c < width; c++){
        ...
        public static void fill(int x, int y, int width, int height, Array<Array<int>> target, int index) {
            int r, c;
        ...
        public static void scatterFill(int x, int y, int width, int height, Array<Array<int>> target, int index, int total, int[]rotations = null, int add = 0) {
            int r, c;
            int rot = 0;
            int breaker = 0;
            while((total--) > 0){
        ...
        public static void placeRandom(int x, int y, int width, int height, Array<Array<int>> target, int index, int avoid = 0) {
            int r, c;
            int breaker = 0;
            do{
                c = x + random.rangeInt(width);
        ...
        /* Scatter some tiles with Math.random */
        public static void scatterFillList(int x, int y, int width, int height, Array<Array<int>> target, Array<int> list) {
            int i, r, c;
            int rot;
            int breaker = 0;
        ...
        public static Array<Array<int>> create2DArray(int width, int height, int basex = -1) {
            int r, c;
            var a = new Array<Array<int>>(new Array<int>[height]);
            for(r = 0; r < height; r++){
                a[r] = new Array<int>(new int[width]);
                for(c = 0; c < width; c++){
                    a[r][c] = basex;
                }
            }
        ...
        public void randomiseArray<T>(Array<T> a) {
            //for(T x = default(T), int j = 0, int i = a.length; i; j = random.rangeInt(i), x = a[--i], a[i] = a[j], a[j] = x){}
        }
